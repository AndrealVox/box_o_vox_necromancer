...
ExecuteSummon.does = 'box_o_vox_necromancer:combat:execute_summon'
ExecuteSummon.args = {
    target = Entity
}
...
function ExecuteSummon:start_thinking(ai, entity, args)
    ...
    self._summon_types = stonehearth.combat:get_combat_actions(entity, 'box_o_vox_necromancer:combat:summon_spells')
    ...
    local equipment_data = stonehearth.combat:get_equipment_data(entity, 'box_o_vox_necromancer:summon_data')
...
function ExecuteSummon:run(ai, entity, args)
    ...
    ai:set_status_text_key('box_o_vox_necromancer:ai.actions.status_text.summoning', {target = target})
...
function ExecuteSummon:_summon(summoner, target, weapon_data)
    ...
    radiant.events.trigger_async(summoner, 'box_o_vox_necromancer:necromancer:has_summoned', { entity = target })
    
end

function ExecuteSummon:_create_summon(summoner, target, summon_uri)
    summon_uri = summon_uri or 'box_o_vox_necromancer:entities:summon:skeleton' -- default summon is skeleton
    ...
    for _, summon in pairs(summons) do
        ...
        radiant.entities.add_buff(summon,'box_o_vox_necromancer:data:buffs:summon_timer')
